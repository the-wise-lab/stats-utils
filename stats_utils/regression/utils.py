...
def run_regression_and_plot(
    ...
    Args:
        data (pd.DataFrame): The data frame containing the variables used in the model.
        var (str): The dependent variable in the regression model.
        predictors (str): A string representing predictor variables in the regression model.
        num_bootstrap_samples (int, optional): The number of bootstrap samples to use for estimating the sampling distribution. Defaults to 2000.
        show_plot (bool, optional): If True, shows the plot. Defaults to True.
        show_summary (bool, optional): If True, shows the model summary. Defaults to True.
    ...
    Returns:
        tuple: Containing:
            model (statsmodels.regression.linear_model.RegressionResultsWrapper): The fitted model object.
            ax (plt.Axes): The Axes object with the plot.
    """
    # 1. Run Regression Analysis
    # smf.ols creates a model from a formula and dataframe
    # f'{var} ~ {predictors}' generates a model formula string. E.g., 'Y ~ X1 + X2'
    model = smf.ols(f"{var} ~ {predictors}", data=data).fit()

    # Display the model summary, which includes things like coefficient values, R-squared, p-values, etc.
    if show_summary:
        print(model.summary())
    ...
    # Perform bootstrap resampling to estimate the sampling distribution of the estimator and calculate confidence intervals
    ...
    if save_fig:
        # Create a file name string, replacing spaces and plus signs with underscores and removing pipe characters
        predictors_str = (
            predictors.replace(" + ", "_").replace(" ", "").replace("|", "or")
        )

        # Create a filename using both dependent and predictor variable names, improving traceability of the saved plot
        ...
        # Adjust the layout to ensure that it looks good and then save the figure as an SVG file in the 'figures' directory
        plt.tight_layout()
    ...
    # 5. Return Axes
    # This will return the ax object, which can be used for further customization outside of the function
    return model, ax
